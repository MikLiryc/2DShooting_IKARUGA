...
image::image() : _imageInfo(NULL), _fileName(NULL)
{
}
...
{
	//재초기화 방지용, 이미지 정보의 값이 들어있다면 릴리즈해서 깨끗하게
...
HRESULT image::initForAlphaBlend()
{
	...
	//알파블렌드 옵션 초기화
	//BlendOP				= 현재는 AC_SRC_OVER 만 사용가능
	//BlendFlags			= 그냥 0 대입. 예전 OS에서 사용했다고 함
	//AlphaFormat			= 24비트 사용시 0, 32비트 사용시 AC_SRC_ALPHA
	//SourceConstantAlpha	= 0 ~ 255  255일때 원본 색상
...
void image::render(HDC hdc, int destX, int destY)
{
	...
	{
		//GdiTransparentBlt : 비트맵 이미지의 특정 색상을 제외하고 복사해주는 함수
		GdiTransparentBlt(
			hdc,				//복사할 장소의 DC
			destX,				//복사할 좌표 시작X
			destY,				//복사할 좌표 시작Y
			...
			_transColor);		//복사할 때 제외할 색상 (일반적으로 마젠타 사용)
	}
	else
...
void image::alphaRender(HDC hdc, BYTE alpha)
{
	...
	if (_isTrans) //배경색 없애고 출력
	{
		//1. 현재화면 DC의 내용을 => 블렌드 이미지에 복사
		//2. 이미지메모리 DC의 배경색을 없앤 후 => 블렌드 이미지에 복사
		//3. 블렌드 이미지를 화면 DC에 복사
...
void image::alphaRender(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight, BYTE alpha)
{
	...
	if (_isTrans)//배경색 없애고 출력
	{
		//1
		BitBlt(_blendImage->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height,
			hdc, _imageInfo->width, _imageInfo->height, SRCCOPY);
